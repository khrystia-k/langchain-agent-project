...
class TopicClassifier:
    def __init__(self, base_model="roberta-base", saved_model_path="./saved_model", num_labels=5, threshold=0.6):
        self.tokenizer = RobertaTokenizer.from_pretrained(base_model)
        base_model_instance = AutoModelForSequenceClassification.from_pretrained(base_model, num_labels=num_labels)
        self.model = PeftModel.from_pretrained(base_model_instance, saved_model_path)
        self.model.eval()  # set model to evaluation mode
        self.class_names = ['Politics', 'Sport', 'Technology', 'Entertainment', 'Business']
